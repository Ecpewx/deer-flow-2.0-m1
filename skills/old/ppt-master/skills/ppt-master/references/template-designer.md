...
- **Output location**: `${SKILL_DIR}/templates/layouts/<template_name>/`
...
When the workflow provides a PPTX reference source, the effective input package comes from the unified `${SKILL_DIR}/scripts/pptx_template_import.py` preparation workspace and becomes:
...
> **Docker path**: All template paths assume `${SKILL_DIR}` as the base. Set `SKILL_DIR=/mnt/skills/public/ppt-master/skills/ppt-master` at the start of execution.
...
```
${SKILL_DIR}/templates/layouts/<template_name>/
...
**Example library structure** (query `${SKILL_DIR}/templates/layouts/layouts_index.json`):

```
${SKILL_DIR}/templates/layouts/
...
- [x] All templates saved to `${SKILL_DIR}/templates/layouts/<template_name>/`
- [x] Templates follow design spec (colors, fonts, layout)
- [x] Placeholder markers are clear and standardized
- [ ] **Next step**: Validate assets and register the template in `${SKILL_DIR}/templates/layouts/layouts_index.json`
```

...
>
> PPT Master requires several Python packages for full functionality. The dependencies are listed in `/mnt/skills/public/ppt-master/requirements.txt`.
>
> ### Docker Environment Setup
>
> **Set SKILL_DIR at the start of execution:**
> ```bash
...
>
> This skill assumes the following Docker mount layout:
...
>
> All commands below use `${SKILL_DIR}` as the base path. If your mount point differs, adjust accordingly.
...
| Icon library | `${SKILL_DIR}/templates/icons/` | Search icons on demand: `ls ${SKILL_DIR}/templates/icons/<library>/ \| grep <keyword>` (libraries: `chunk/`, `tabler-filled/`, `tabler-outline/`) |
...
> 💡 **Docker hint**: Projects are created under the current working directory by default. To place them under `/mnt/user-data/projects/`, add `--dir /mnt/user-data/projects`:
> ```bash
> python3 ${SKILL_DIR}/scripts/project_manager.py init <project_name> --format <format> --dir /mnt/user-data/projects
> ```
...
> To create a new global template, read `${SKILL_DIR}/workflows/create-template.md`
...
> ⚠️ **Mandatory gate in `strategist.md`**: Before writing `design_spec.md`, Strategist MUST `read_file ${SKILL_DIR}/templates/design_spec_reference.md` (or `<project_path>/templates/design_spec_reference.md` if project-level template exists) and produce the spec following its full I–XI section structure. See `strategist.md` Section 1 for the explicit gate rule.
...
- `<project_path>/spec_lock.md` — machine-readable execution contract (distilled from the decisions in design_spec.md; Executor re-reads this before every page). See `${SKILL_DIR}/templates/spec_lock_reference.md` for the skeleton.
...
```
Read ${SKILL_DIR}/references/executor-base.md          # REQUIRED: common guidelines
Read ${SKILL_DIR}/references/executor-general.md       # General flexible style
Read ${SKILL_DIR}/references/executor-consultant.md    # Consulting style
Read ${SKILL_DIR}/references/executor-consultant-top.md # Top consulting style (MBB level)
```
...
# Output: <project_path>/exports/<project_name>_<timestamp>.pptx + <project_path>/exports/<project_name>_<timestamp>_svg.pptx
...
| Resource | Path |
|----------|------|
| Shared technical constraints | `${SKILL_DIR}/references/shared-standards.md` |
| Canvas format specification | `${SKILL_DIR}/references/canvas-formats.md` |
| Image layout specification | `${SKILL_DIR}/references/image-layout-spec.md` |
| SVG image embedding | `${SKILL_DIR}/references/svg-image-embedding.md` |

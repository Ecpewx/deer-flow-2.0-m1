...
- For those solid arrows, draw a standalone closed `<path>` / `<polygon>` and reference `${SKILL_DIR}/templates/charts/chevron_process.svg` or `${SKILL_DIR}/templates/charts/process_flow.svg`.
...
> Arrows: prefer `marker-end` with a qualifying `<marker>` (see §1.1) for connector lines — the converter produces native DrawingML arrow heads that auto-rotate. For block arrows / chunky arrows, use a standalone closed shape instead of `marker`; see `${SKILL_DIR}/templates/charts/chevron_process.svg` for phase arrows and `${SKILL_DIR}/templates/charts/process_flow.svg` for mixed flow layouts.

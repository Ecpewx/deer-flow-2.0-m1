...
Defined in the Design Specification & Content Outline; each image has a status annotation. If the image approach includes "B) User-provided", you must run `${SKILL_DIR}/scripts/analyze_images.py` immediately after the Strategist completes the Eight Confirmations, and complete the list before outputting the design spec.
...
```bash
python3 ${SKILL_DIR}/scripts/finalize_svg.py <project_path>         # Icons, images, text, rounded rects — all in one pass
python3 ${SKILL_DIR}/scripts/svg_to_pptx.py <project_path> -s final  # Export PPTX from final version
```
...
```bash
python3 ${SKILL_DIR}/scripts/svg_finalize/embed_images.py <svg_file>                         # Single file
python3 ${SKILL_DIR}/scripts/svg_finalize/embed_images.py <project_path>/svg_output/*.svg    # Batch
python3 ${SKILL_DIR}/scripts/svg_finalize/embed_images.py --dry-run <project_path>/svg_output/*.svg  # Preview
```

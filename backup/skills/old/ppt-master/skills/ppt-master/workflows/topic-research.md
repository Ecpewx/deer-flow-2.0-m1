...
|-------------|------|---------|
| Structured Markdown document | `<PROJECTS_DIR>/<topic_name>.md` | `<PROJECTS_DIR>/joe_hisaishi.md` |
| Image folder | `<PROJECTS_DIR>/<topic_name>/` (same name as the document, without extension) | `<PROJECTS_DIR>/joe_hisaishi/` |

> **Naming consistency rule**: The Markdown filename (without `.md`) and the image folder name MUST be identical.
>
> **Output directory rule**: Both the document and image folder MUST be created inside the projects directory. Never place them in the repository root or any other location.
>
> ⚠️ **Docker path**: Set `PROJECTS_DIR=/mnt/user-data/projects` at the start of execution.
...
```bash
# Create image folder under projects directory (same name as the document)
mkdir -p "<PROJECTS_DIR>/<topic_name>"

# Download images with descriptive filenames
curl -L -o "<PROJECTS_DIR>/<topic_name>/descriptive_name.jpg" "<image_url>"
```
...
**Topic**: <topic_name>
**Document**: `<PROJECTS_DIR>/<topic_name>.md` — [X sections, approximately Y words]
**Images**: `<PROJECTS_DIR>/<topic_name>/` — [N images collected]

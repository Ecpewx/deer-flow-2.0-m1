...
As a top-tier AI presentation strategist, receive **pre-confirmed content overview** and template selection, then perform **design/style planning** and output the **Design Specification & Content Outline** (hereafter `design_spec`).

> 💡 **Content is already confirmed**: The user has reviewed and approved a page-by-page content overview in Step 3. The Strategist's job is to take this confirmed content structure and wrap it in professional design decisions — colors, typography, layout, iconography, imagery.

## Pipeline Context

| Previous Step | Current | Next Step |
|--------------|---------|-----------|
| Template confirmed (Step 4) | **Strategist**: Style/Design Eight Confirmations (Step 5) | Image_Generator or Executor |
...
```
read_file <project_path>/templates/design_spec_reference.md     # If project has templates
read_file ${SKILL_DIR}/templates/design_spec_reference.md       # Fallback: global template
```
...
⛔ **BLOCKING**: Provide professional recommendations for the following eight items, then **present them as a bundled package to the user and wait for explicit confirmation or modifications**.

> **Execution discipline**: This is the last (third) BLOCKING checkpoint in the pipeline (after content overview and template selection). Once the user confirms, the AI must automatically complete the Design Specification & Content Outline and seamlessly proceed to subsequent image generation (if applicable), SVG generation, and post-processing — no additional questions or pauses in between.
...
Confirm page count aligns with the pre-confirmed content overview (Step 3). The page structure from `<project_path>/content_overview.md` should be the foundation. Provide any adjustments if needed (e.g., merging sparse pages, splitting dense ones).
...
**Industry color quick reference** (full 14-industry list in `${SKILL_DIR}/scripts/config.py` under `INDUSTRY_COLORS`):
...
> 2. Search for icon availability: `ls ${SKILL_DIR}/templates/icons/<chosen-library>/ | grep <keyword>`
...
**When selection includes B**, you must run `python3 ${SKILL_DIR}/scripts/analyze_images.py <project_path>/images` before outputting the spec, and integrate scan results into the image resource list.
...
**Image-layout alignment principles** (detailed calculation rules in `${SKILL_DIR}/references/image-layout-spec.md`):
...
> **Portrait canvases** (Xiaohongshu, Story): Layout rules differ — top-bottom is preferred for most ratios since left-right columns become too narrow. See "Portrait Canvas Override" in `${SKILL_DIR}/references/image-layout-spec.md`.

> **Multi-image slides**: When multiple images appear on one page, use the grid formulas in the "Multi-Image Layout" section of `${SKILL_DIR}/references/image-layout-spec.md`.
...
When content pages involve **data visualization or infographic-style structured information design** (comparisons, trends, proportions, KPIs, flows, timelines, org structures, strategic frameworks, etc.), Strategist should select appropriate visualization types from the built-in template library.

> **Mandatory first step**: At the beginning of content planning, **read the full `${SKILL_DIR}/templates/charts/charts_index.json`** file. This index contains all available visualization templates (52 types across 8 categories), including each template's `summary`, `bestFor`, `avoidFor`, and `keywords`. Strategist must internalize the full catalog before making selections — do NOT rely on memory or partial lists.

> **Selection workflow**:
> 1. Read and internalize the complete `${SKILL_DIR}/templates/charts/charts_index.json`
> 2. For each page in the pre-confirmed content overview (`<project_path>/content_overview.md`), determine whether it needs visualization based on its information structure
...
The Strategist should make professional judgments on the template basis generated by `${SKILL_DIR}/scripts/project_manager.py`, considering user needs, content characteristics, and audience:
...
| IX. Content Outline | **Inherits from pre-confirmed content overview** (`<project_path>/content_overview.md`). Preserve the confirmed page structure. Each page includes layout, title, content points, visualization type (if applicable). Add design annotations (layout mode, visual type) on top of the confirmed content. |
...
**Generation steps**:
1. Read the pre-confirmed content overview: `read_file <project_path>/content_overview.md`
2. Read reference template: `<project_path>/templates/design_spec_reference.md` (or `${SKILL_DIR}/templates/design_spec_reference.md` if no project template)
3. Generate complete spec from scratch based on analysis, inheriting the confirmed content structure
4. Save to: `<project_path>/design_spec.md`
5. **Generate execution lock**: read `${SKILL_DIR}/templates/spec_lock_reference.md` and produce `<project_path>/spec_lock.md` — a distilled, machine-readable short form of the color / typography / icon / image decisions above. This file is what the Executor re-reads before every page (see [executor-base.md](executor-base.md) §2.1). The values in `spec_lock.md` MUST exactly match the decisions recorded in `design_spec.md`; if they ever diverge, `spec_lock.md` wins and `design_spec.md` should be treated as historical narrative
...
```bash
python3 ${SKILL_DIR}/scripts/project_manager.py init <project_name> --format <canvas_format> --dir /mnt/user-data/projects
```

> 💡 **Docker path**: Projects are under `/mnt/user-data/projects/`. Set `SKILL_DIR=/mnt/skills/public/ppt-master/skills/ppt-master`.

The Strategist saves the Design Specification & Content Outline to `<project_path>/design_spec.md`.

---

## 8. Complete Design Spec and Prompt Next Steps

After writing `design_spec.md` **and** `spec_lock.md`, provide the next-step prompt based on the confirmed image usage selection. This prompt is a workflow handoff instruction, not a section inside `design_spec.md`.
...
> 💡 Template option was already confirmed in Step 4. The appropriate executor style (General / Consultant / Top Consultant) has been determined through the style confirmation above.

...
```bash
python3 ${SKILL_DIR}/scripts/image_gen.py "your prompt" \
  --aspect_ratio 16:9 --image_size 1K \
  --output <project_path>/images --filename cover_bg
```
...
|----------|----------|-------------|
| `IMAGE_BACKEND` | Required | Backend identifier; run `${SKILL_DIR}/scripts/image_gen.py --list-backends` for the current set |
...
3. Remove watermark: `python3 ${SKILL_DIR}/scripts/gemini_watermark_remover.py <image_path>`

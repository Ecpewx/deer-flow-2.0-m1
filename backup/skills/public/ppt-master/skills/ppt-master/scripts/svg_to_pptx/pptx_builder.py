...
def create_pptx_with_native_svg(
    ...
    try:
        ...
        failed_pages: list[tuple[int, Path, Exception]] = []
        retried_pages: set[int] = set()
        fallback_pages: set[int] = set()

        # ========== Helper: write a native-mode slide ==========
        def _write_native_slide(svg_path: Path, slide_num: int) -> bool:
            """Try to convert and write a native-shape slide. Returns True on success."""
            nonlocal has_any_image
            try:
                slide_xml, media_files_dict, rel_entries = convert_svg_to_slide_shapes(
                    svg_path, slide_num=slide_num, verbose=False,
                )
                ...
                for rel in rel_entries:
                    ...
                    if not target.startswith('../media/'):
                        continue
                    orig_name = target.split('../media/', 1)[1]
                    mapped_name = media_name_map.get(orig_name)
                    if mapped_name:
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/slideLayout" Target="../slideLayouts/slideLayout1.xml"/>{extra_rels}
        ...
        # ========== Helper: write a legacy (SVG image) mode slide ==========
        def _write_legacy_slide(svg_path: Path, slide_num: int) -> bool:
            """Embed SVG as image on the slide. Returns True on success."""
            nonlocal has_any_image
            try:
        ...
        # ========== Helper: attach notes to a slide ==========
        def _attach_notes(svg_path: Path, slide_num: int, rels_path: Path) -> bool:
            """Attach speaker notes to a slide. Returns True if notes were added."""
            if not enable_notes:
                return False
        ...
        for i, svg_path in enumerate(svg_files, 1):
            ...
            try:
                if use_native_shapes:
                    ok = _write_native_slide(svg_path, slide_num)
                else:
                    ok = _write_legacy_slide(svg_path, slide_num)

                # Attach notes
                rels_dir = extract_dir / 'ppt' / 'slides' / '_rels'
                rels_path = rels_dir / f'slide{slide_num}.xml.rels'
                _attach_notes(svg_path, slide_num, rels_path)

                if verbose:
                    mode_str = " (Native)" if use_native_shapes else (" (PNG+SVG)" if use_compat_mode else " (SVG)")
                ...
                failed_pages.append((slide_num, svg_path, e))
                if verbose:
                    print(f"  [{i}/{len(svg_files)}] {svg_path.name} - Error (Page {slide_num}): {e} [will retry]")

        # ============================================================
        # RETRY PASS: Attempt failed pages
        # ============================================================
        if failed_pages:
            if verbose:
                print()
                print(f"  {'=' * 55}")
                print(f"  ⚠️  {len(failed_pages)} page(s) failed. Running auto-retry...")
                print(f"  {'=' * 55}")

            for slide_num, svg_path, orig_err in failed_pages:
                retried = False

                # Retry 1: Try native mode again (in case of transient errors)
                if use_native_shapes:
                    try:
                        _write_native_slide(svg_path, slide_num)
                        rels_dir = extract_dir / 'ppt' / 'slides' / '_rels'
                        rels_path = rels_dir / f'slide{slide_num}.xml.rels'
                        _attach_notes(svg_path, slide_num, rels_path)
                        retried = True
                        retried_pages.add(slide_num)
                        success_count += 1
                        if verbose:
                            print(f"  [Retry ✓] Page {slide_num}: {svg_path.name} - Native conversion succeeded on retry")
                    except Exception:
                        pass

                # Retry 2: Fall back to SVG image embedding
                if not retried:
                    try:
                        _write_legacy_slide(svg_path, slide_num)
                        rels_dir = extract_dir / 'ppt' / 'slides' / '_rels'
                        rels_path = rels_dir / f'slide{slide_num}.xml.rels'
                        _attach_notes(svg_path, slide_num, rels_path)
                        fallback_pages.add(slide_num)
                        success_count += 1
                        if verbose:
                            print(f"  [Retry ⚠️] Page {slide_num}: {svg_path.name} - Fallback to SVG image mode (not editable)")
                    except Exception as e2:
                        if verbose:
                            print(f"  [Retry ✗] Page {slide_num}: {svg_path.name} - Still failed: {e2}")
        ...
        if verbose:
            ...
            print(f"  Succeeded: {success_count}/{len(svg_files)} slides")

            # Retry summary
            if retried_pages:
                pages_str = ', '.join([f'#{p}' for p in sorted(retried_pages)])
                print(f"  ✅ Retry success: Page(s) {pages_str} converted on second attempt")
            if fallback_pages:
                pages_str = ', '.join([f'#{p}' for p in sorted(fallback_pages)])
                print(f"  ⚠️  Fallback: Page(s) {pages_str} used SVG image mode (not directly editable)")
            failed_count = len(svg_files) - success_count
            if failed_count > 0:
                print(f"  ❌ Failed: {failed_count} slide(s) could not be converted (see errors above)")
            else:
        ...
        # Also store retry results as instance vars for downstream use
        result = success_count == len(svg_files)
        return result

#!/usr/bin/env python3
"""
DeerFlow 课程论文选题生成 测试工具
====================================
基于 course-paper-topic skill，通过 Gateway API：
  1. 创建对话线程
  2. 上传课件/参考资料
  3. 调用 DeepSeek Reasoner 生成论文选题
  4. 提取生成结果
  5. 可选：导出 DOCX

用法：
  # 使用默认课件和提示词
  python3 scripts/test_paper_topic.py

  # 指定课件
  python3 scripts/test_paper_topic.py -f ~/Desktop/精算学原理/A2\ 完整课件/第三章-生命表.pptx

  # 多文件（课件 + 参考文献 + 教材）
  python3 scripts/test_paper_topic.py -f 课件.pptx 教材.pdf 参考文献.docx

  # 自定义提示词
  python3 scripts/test_paper_topic.py -p "根据课件生成5个本科课程论文选题，要求紧密结合精算实务"

  # 导出 DOCX
  python3 scripts/test_paper_topic.py --export-docx
"""
...
def _set_gateway(url):
    global GATEWAY
    GATEWAY = url
DEFAULT_COURSEFILE = os.path.expanduser(
    "~/Desktop/精算学原理/A2 完整课件/第三章-生命表.pptx"
)
DEFAULT_OUTPUT_DIR = os.path.expanduser("~/Desktop/精算学原理/论文选题输出")
DEFAULT_PROMPT = (
    "根据我上传的课件，帮我生成一份课程论文选题方案。\n"
    "要求：\n"
    "1. 生成 8-10 个选题，涵盖理论分析、实证研究、案例分析等不同类型\n"
    "2. 每个选题包含：题目、研究类型、难度等级（★~★★★★★）、选题说明（200字以上）、"
    "建议研究方法、推荐参考文献方向\n"
    "3. 选题应符合本科/研究生课程论文水平\n"
    "4. 确保选题具有学术价值和可操作性\n"
    "5. 输出为结构化的 Markdown 文件"
)
...
def stream_run(thread_id: str, message: str):
    """发送 run/stream 请求，实时显示进度，返回 events 列表。"""
    body = {
...
def extract_outputs(events: list, thread_id: str, output_dir: str):
    """从 SSE 事件或线程 outputs 目录中提取生成的文件。"""
    base = os.path.join(
        os.path.dirname(__file__),
        "..", "backend", ".deer-flow", "threads",
        thread_id, "user-data", "outputs",
    )
...
def export_docx(md_files: list, output_dir: str):
    ...
    export_script = os.path.join(
        os.path.dirname(__file__),
        "..", "skills", "public", "course-paper-topic", "scripts", "export.py",
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="DeerFlow 课程论文选题生成测试工具",

...
def _record_to_batch_progress(record: RunRecord) -> BatchRunProgressResponse:
    metadata = record.metadata or {}
    return BatchRunProgressResponse(
        run_id=record.run_id,
        thread_id=record.thread_id,
        status=record.status.value,
        batch_mode=bool(metadata.get("batch_mode", False)),
        stage=str(metadata.get("batch_stage", "idle")),
        total=int(metadata.get("batch_total", 0) or 0),
        processed=int(metadata.get("batch_processed", 0) or 0),
        succeeded=int(metadata.get("batch_succeeded", 0) or 0),
        failed=int(metadata.get("batch_failed", 0) or 0),
        error=record.error,
    )
...
@router.get("/{thread_id}/runs/{run_id}/progress", response_model=BatchRunProgressResponse)
async def get_run_progress(thread_id: str, run_id: str, request: Request) -> BatchRunProgressResponse:
    """Get a normalized progress view for long-running (especially batch) runs."""
    run_mgr = get_run_manager(request)
    record = run_mgr.get(run_id)
    if record is None or record.thread_id != thread_id:
        raise HTTPException(status_code=404, detail=f"Run {run_id} not found")
    return _record_to_batch_progress(record)

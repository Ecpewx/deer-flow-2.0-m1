...
#
# 🔄 Auto-retry on error:
#   1st retry: Native conversion again (transient error recovery)
#   2nd retry: Fallback to SVG image embedding (page preserved, not editable)
#   Final PPTX always contains ALL pages, reports status per slide
```

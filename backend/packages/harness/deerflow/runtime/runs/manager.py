...
class RunManager:
    ...
    async def update_metadata(self, run_id: str, updates: dict) -> bool:
        """Merge metadata updates into an existing run record.

        Returns:
            True when the run exists and metadata is updated, False otherwise.
        """
        if not updates:
            return False
        async with self._lock:
